...
/// <summary>
/// Shared PDF content-stream inspection helpers for barcode tests (mirrors
/// <c>VellumPdf.Layout.Tests.PdfTestUtil</c>'s decompress-and-regex approach).
/// </summary>
internal static class PdfTestUtil
{
    /// <summary>
    /// Scans <paramref name="pdf"/> for FlateDecode stream data, decompresses each stream, and
    /// concatenates the results as Latin-1 text for assertion. Uses the <c>/Length N</c> field
    /// from each stream dict to slice exact bytes so compressed content bytes that happen to
    /// contain the "endstream" token don't confuse the parser.
    /// </summary>
