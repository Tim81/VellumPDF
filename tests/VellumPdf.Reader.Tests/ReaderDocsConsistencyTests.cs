...
/// <summary>
/// The Reader capability table is published twice, deliberately: once in
/// <c>src/VellumPdf.Reader/README.md</c>, which is what the NuGet listing shows, and once in
/// <c>docs/reader-guide.md</c>, alongside the narrative walkthrough it summarizes. Duplication only
/// stays honest if something keeps the two from drifting apart, so both copies are wrapped in
/// matching <c>&lt;!-- capability-table:reader:start/end --&gt;</c> markers, and this test asserts
/// the marked span is byte-identical between them — an edit to one copy without the other fails
/// here instead of shipping a README and a guide that disagree.
/// </summary>
public sealed class ReaderDocsConsistencyTests
{
    [Fact]
    public void CapabilityTable_readmeAndGuide_areByteIdentical()
    {
        ...
        var readmeTable = ExtractMarkedBlock(
            Path.Combine(root, "src", "VellumPdf.Reader", "README.md"), "reader");
        var guideTable = ExtractMarkedBlock(
            Path.Combine(root, "docs", "reader-guide.md"), "reader");
