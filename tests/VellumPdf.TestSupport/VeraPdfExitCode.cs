...
/// against veraPDF 1.30.2: 0 valid, 1 non-compliant, 2 a rejected <c>--flavour</c> argument, 3 a
/// job veraPDF abandoned part-way, 4 no file found, 7 a file veraPDF could not parse, 8 one it
/// refused as encrypted.
...
public static class VeraPdfExitCode
{
    public static string Describe(int exitCode) => exitCode switch
    {
        ...
        3 => "veraPDF abandoned the job rather than reaching a verdict: read its own stderr, which "
             + "names the cause. Measured once here, on a /RowSpan of int.MaxValue, where it tried "
             + "to allocate a row array of that size and died with \"Requested array size exceeds "
             + "VM limit\". Neither a pass nor a fail, so a test that only checks for 1 would read "
             + "this as a defect in the wrong place.",
