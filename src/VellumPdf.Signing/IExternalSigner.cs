...
public interface IExternalSigner
{
    ...
    /// RSA certificate is always an RSASSA-PKCS1-v1_5 algorithm identifier
    /// (<c>sha256WithRSAEncryption</c>/<c>sha384WithRSAEncryption</c>/<c>sha512WithRSAEncryption</c>),
    /// never a PSS one, so a PSS signature (Azure Key Vault <c>PS256</c>/<c>PS384</c>/
    /// <c>PS512</c>, AWS KMS <c>RSASSA_PSS_SHA_256</c> and similar) fails verification even
    /// though the signer itself succeeded — configure the KMS/HSM key for PKCS#1 v1.5
    /// signing instead. For an EC certificate, the DER-encoded <c>ECDSA-Sig-Value</c> sequence, not a raw
