...
internal static class PdfFilters
{
        ...
        DiagnosticSink? diagnostics, int? objectNumber, int? generation)
    {
        ...
        // is what makes this severity Error rather than Warning. The name is excerpted in both
        // places, not interpolated whole: a /Filter name has no length bound (Annex C.1), and this
        // dictionary's filter object is dereferenced once and shared by every stream that resolves
        ...
        // PdfPreflight's per-rule catch (bounded at the sink by #403) keeps a copy of the
        ...
        throw new InvalidDataException($"Unknown PDF filter: /{DiagnosticExcerpt.Quote(filter.Value)}");
    }
