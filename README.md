...
| `VellumPdf.Conformance` | Preview | In-process PDF/A-2b/2u/2a and PDF/UA-1 preflight: runs clean-room conformance rules authored from the ISO specifications and returns machine-readable assertions (rule id, ISO clause, severity, object reference) — no external veraPDF Docker image needed. AOT- and trim-ready (rules registered explicitly, no reflection). The rule set is a growing structural subset (several rules cover the common cases and document their deferred edges); the PDF/A-2b structural and metadata fixtures are cross-validated against veraPDF in CI, with object-graph and 2u/2a/UA cross-validation in progress. |
