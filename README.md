...
| `VellumPdf.Reader` | [![NuGet](https://img.shields.io/nuget/v/VellumPdf.Reader.svg?label=%20)](https://www.nuget.org/packages/VellumPdf.Reader) | Preview | Opens existing PDFs (classic cross-reference tables, cross-reference and object streams, hybrid-reference files, and encrypted documents given their password) and exposes the catalog, signatures, and decoded stream data. The basis for the signing LTV path, the conformance validator, and a general reader. |
