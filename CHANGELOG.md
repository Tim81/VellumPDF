...
- **The acknowledgement the GIF licence requires, packed into every package.** Clause 3 of the
  ...
  never `NOTICE`. All eight packages shipped GIF code with no acknowledgement, not seven: a
  package carries its own assembly plus a dependency on the Kernel, not the Kernel's own code, so
  depending on the package that holds the codec was never an exemption. `NOTICE` now packs into
  all eight and carries the full text. The Kernel package, the only one containing a GIF decoder
  or encoder, also carries a short acknowledgement in its own README; the root README and the
  other seven package READMEs do not, since every package ships `NOTICE` regardless of which one
...
- **The GIF decoder refused a third of the files put to it and silently corrupted others
  ...
  Nothing that decoded correctly before decodes differently now. This sweep ran outside the
  repository against an external corpus; no generator or corpus for it is checked in under `eng/`,
  so the table above is not reproducible from a clone as it stands.
...
- **A Graphic Control Extension's transparency leaked past its own scope (#490).** Section 23
  ...
  sorts every block label into the three ranges a decoder needs in order to find that scope even
