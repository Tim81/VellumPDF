...
- **A form field's font size, and the coordinates written alongside it, followed the host
  ...
  three wrote a complete file with a broken operand instead of raising. The refusal reaches
  further than the decimal point: measured across the 890 cultures this runtime offers, 886
  spell positive infinity as U+221E, which a Latin-1 content stream can only represent as `?`,
  and 93 spell not-a-number differently from the invariant `NaN`. Twenty-six of those write a word
...
- **Three reviews of the layout boundary-documentation sweep (#503) found the rule it shipped
  ...
  `<exception>` tags.** `CONTRIBUTING.md` said `<remarks>` opens with a bold sentence and a
  paragraph headed "Do not pass ..."; neither is true of the tree, and `TextStyle.FontSize`, the
  rule's own example, opens with a plain sentence and bolds only `not`. The rule in
  `CONTRIBUTING.md`, `CLAUDE.md`, and the CHANGELOG entry that shipped it now describe that
  pattern instead. `CONTRIBUTING.md`'s derivation example moved from `NotSupportedException` and
  `InvalidDataException`, neither of which appears together in Layout, to `Document.Save`'s
  `ArgumentException` and `ArgumentOutOfRangeException`, where the second actually derives from
  the first.

- Corrected false statements found by probe: `Document.Margins` said a non-finite inset "is
  refused in only one of its three forms," though all three throw, each by a different route;
  `Document.PageSize` said changing it after adding elements "does not re-lay out what you
  already added," though 12 paragraphs built at 600x800pt and resized to 200x120pt before save
  produce a file byte-identical (bar the random `/ID`) to building at 200x120pt from the start;
  ...
  and its cell-padding claim is bounded to a `Left`-heavy inset, since the same total split across
  `Left` and `Right` lands inside the page instead of outside it.

- Corrected two ISO 32000-2 citations. `Heading` claimed NOTE 2 to Table 366 "says outright that
  `H7` may be used"; the NOTE is informative, not a requirement, and says `H7` "can" be used.
  ...
  used`. The clause also requires a non-negative line width, so a negative one is the format's
  business, not only the renderer's, as the member wrongly said; whether to refuse it is being
  decided in #482.

- Added `<exception>` tags found by probe: `TextStyle.FontSize`'s reachable `ArgumentException`,
  raised when a `RunningBand` style's positive-infinity font size inflates the band past the page
  (measured: `NaN` on the same style instead reaches the generic too-tall message, so the tag's
  "names the element and the size" claim now says which non-finite values that covers rather than
  claiming all three); `Document.PageSize`'s `ArgumentException` (page smaller than the margins)
  and `InvalidOperationException` (page too small for its own content); `Document.Save(string)`
  and `SaveAsync(string, CancellationToken)`, which carried none while their stream siblings
  carried four each; `PieChart.Diameter`'s `InvalidOperationException`, since its clamp compares
  only against the available width, so a diameter that fits once clamped can still be taller than
  the page has room for; and `Cell.ColSpan`'s `OverflowException` and `OutOfMemoryException`, both
  reachable at `int.MaxValue`. `RunningBand.Height`'s `ArgumentException` condition widened from
...
- Voice and cross-reference pass over the members this touched. `Attention:` cut from three
  ...
  `LayoutImage.Height`'s aspect-override note). Five bold clauses on `LayoutImage` and `Cell`
  reduced to one bolded word each, and `Cell.ColSpan` and `.RowSpan` brought to matching phrasing.
  Of the file's two misapplied `NOTE:` prefixes, one became `Attention:`, since it named a trap,
  and one was dropped, since it stated a plain fact. `RunningBand.Height`'s cross-reference to "the
  string overloads" named only `Document.Save(string)`; it now names
  `SaveAsync(string, CancellationToken)` too. `SaveAsync(string, CancellationToken)`'s own remarks
  pointed at the synchronous stream overload instead of its async sibling. Two save overloads
  described a path already holding a file as "saved again," which reads as a second call on the
  same `Document`, though a `Document` is single-use; reworded to a fresh `Document` given an
  already-occupied path. `ListItem.Children`'s "Be aware that" filler is gone.

  `RunningBand.Height`'s own refusal claim is left alone. It says a non-finite value is refused,
...
- **Text extraction from simple fonts (#98).** `PdfDocumentReader.ExtractText()` and
  ...
  predefined CMaps, `/ActualText`, `/ReversedChars`, and word- and paragraph-aware line grouping
  are not implemented yet. Concretely, the word-aware gap means no space is synthesised for a
  positioning gap: `[(Hello) -500 (World)] TJ` extracts as `HelloWorld`, not `Hello World`. Line
  grouping IS rotation-aware: a rotated page's baseline stays one line rather than opening a new
  run per glyph. Positioned by ISO 32000-2 §9.4.4's text-space matrix arithmetic (`Trm`, and the
  per-glyph and `TJ` advances) and joined into lines wherever the baseline changes — a font set by
  `Tf` before a Form XObject with its own `/Resources` is invoked, and never re-set inside the
  form, resolves against the resources in effect when `Tf` ran, not the form's, per §9.3.1
  Table 103. An ExtGState's own `/Font` (§8.4.5 Table 57) sets font and size the same way, without
  needing a `Tf` at all; Table 57's own text requires its first element to be an indirect
  reference to a font dictionary, but a direct font dictionary there is accepted and used too,
  since it resolves the same way and rejecting it would trade a working font binding for an
  unresolvable one. Word spacing applies only to a single-byte code 32 per §9.3.3; a code with no
  Unicode route still advances the text matrix so later glyphs keep their position, contributing
  no character. Annotation appearance streams are excluded, unlike `ExtractImages()`: §12.5.5
  makes an appearance a rendering convenience layered on the page, and extraction draws that line
  for text where it does not for images. Four new diagnostic codes: `ExtGStateFontMalformed` (310,
  an ExtGState `/Font` array not matching Table 57's own shape), `FontTypeUnsupported` (405, a font
  this reader cannot decode yet), and, in a new `6xx` text-extraction block,
  `TextExtractionLimitExceeded` (600) and `TextShownWithoutFont` (601, a show operator before any
  `Tf`).
...
- **A preflight finding could retain a producer-sized string for the result's lifetime.** A rule
  ...
  tracked in #406. (#403)
- **`PdfFilters.ApplyFilter`'s unknown-filter exception, and eleven other Reader exception messages
  ...
  | Site | Exception |
  | --- | --- |
  | `PdfFilters.ApplyFilter`'s unknown-filter throw | `InvalidDataException` |
  | `PdfObjectParser.ParseIndirectObject`'s `'obj'` keyword mismatch | `InvalidDataException` |
  | `PdfObjectParser.ProbeIndirectObjectHeader`'s `'obj'` keyword mismatch | `InvalidDataException` |
  | `PdfObjectParser.ParseKeywordObject`'s unexpected-keyword throw | `InvalidDataException` |
  | `PdfObjectParser.ExpectEndobj`'s `'endobj'` mismatch | `InvalidDataException` |
  | `PdfObjectParser.ParseReal`'s malformed-real throw | `InvalidDataException` |
...
- **An image with an explicit `Width` wider than its content box was drawn past its right edge.**
  ...
  clamping `Width` alone is already as close to that as the box allows. The clamp converts a throw
  into output too: a `Width` of 600 on this same page, `Height` left null so it follows the clamped
  width, threw `ElementTooTall` before this fix — the unclamped square reservation exceeded the
  400pt content height — and now renders at the clamped 300pt width. No document that already
  rendered moves, since the prior outcome there was an exception rather than output a caller was
  relying on. Guarded on a positive available width: ordinary margins wider than the content box
  reach a non-positive width too, not only the negative insets v3.0 still defers. The XML doc on
  `LayoutImage.Width` now says the value is an upper bound rather than a guaranteed size.

- **A pie chart with a `Diameter` wider than its content box was drawn past it.** `PieChartRenderer`
  positioned a wedge circle at the caller's `Diameter` with no check against the space `Layout` was
  handed. Measured on a 400x900pt page, 50pt margins (content box [50, 350]), the chart's own
  `Margins` set to zero: a centred `Diameter` of 320 gave an operand extent of [40, 360], twenty
  points wider than the box — ten points past on each side. The diameter used to place and draw the
  chart is now clamped to `ctx.Area.Width`, the area `Layout` was handed, before the chart's own
  margins (6pt by default) deflate it. With the default margins, a `Diameter` of 300 already spans
  exactly [50, 350] in a 300pt content box, a correct document, and clamping against the *deflated*
  288pt width instead would have moved it to [56, 344] regardless. Clamping the diameter is not
  sufficient on its own, and only centred charts hide that: Left and Right take their offset from
  the area the chart's own margins deflate, so a circle clamped to the content box is then shifted
  out of it by a margin. Measured on the same page, Left gave [56, 356] and Right [44, 344]; on a
  454.4pt page with a 1.2pt document margin, each ran 4.8pt off the page itself. So the circle is
  ...
  page, though: a `Diameter` of 300 with the chart's default margins on a 200x220pt page threw
  `ElementTooTall` before this fix and now renders a 180pt circle, clamped to the content width. No
  correct document moves either way, because the prior outcome there was an exception rather than
  output a caller was relying on. The XML doc on `PieChart.Diameter` now says so.
...
- **An unbreakable paragraph glyph wider than its box was placed left of the box under Centre or
  ...
  proof beside it; here, a single over-wide rune is proven capable of exceeding it, and the floor
  asserts nothing false. The precondition really is one rune, not a word: `HardBreakWord` flushes
  whenever a fragment plus the next character would exceed the box, so every line it emits stays
...
- **A list marker wider than its indent overprinted the item's own text.** The marker paragraph has
  ...
  items 1 through 26 keep their exact indent, and the left text edge turns ragged from item 27 on —
  though not every later item moves: items 29, 30, 31 and 40 also keep the exact 20pt indent, a
  caller-visible cost of the fix limited to the items whose marker genuinely widens. The nested arm
  had the same defect at `indent * 2` and is widened on the same principle, in a different form,
  needing 27 or more children of one parent to reach it, since nested ordered markers restart at 1
  per parent. Neither widening can push a content paragraph's own width to zero or below: a marker
  as wide as the whole item area keeps today's indent and overprints, rather than widening until
  the item's text lays out nothing at all. The fix works in advance width, because this library
  ships advance widths and no glyph bounding boxes, so a marker whose advance exceeds the indent
  while its ink does not will also move: a bullet list with `Indent = 3` moves every item's content
  by 0.5pt, since the bullet's own advance is 3.5pt. That is a deliberate trade rather than an
  oversight.

- **A 211-document before-and-after corpus confirms the image, chart and list-marker fixes above
  move only what they mean to move.** The harness comparing rendered output before and after this
  branch was itself wrong when an earlier figure was written: it resumed its scan one character
  into `endstream`, matching the tail of the keyword it had just consumed and silently dropping
  every second stream. Fixed and re-run over the full 211-document set, six documents differ, and
  every differing row keeps its text-run count, so nothing lost text. The six are the chart under
  Left and Right at a diameter equal to the content box, the image under Centre and Right at a
  width whose own margins put it left of the box, a bullet list with a 3pt indent, and a roman list
...
- **Cited FIPS 197, FIPS 180-4, FIPS 186-5, Adobe TN5014, ITU-T T.81 and ITU-T T.6 in code that
  predates having a copy of any of them.** The R6 key-derivation hash in `StandardSecurityHandler`
  now points at FIPS 197 §5 for the fixed 128-bit AES block size that lets a 16-byte IV cover both
  the AES-128 step inside the hash and the AES-256 encryption around it, and at FIPS 180-4 §1 for
  the three digest sizes its SHA-256/384/512 branch chooses between. `EcdsaSignatureConverter`
  points at FIPS 186-5 §6.4.1 for the pair-of-integers signature shape it converts between P1363
  and DER. `CffFont` and `TrueTypeFontEmbedder` point at Adobe TN5014 §4.2 for what a CFF's ROS
  operator and a CIDFont's `/CIDSystemInfo` actually identify, and TN5014 §7.4 for the 100-line cap
  the ToUnicode CMap writer already chunked to without saying why. Left uncited: AES itself, which
  ...
  here. `JpegImageLoader` now points at ITU-T T.81 Table B.1 for why its SOF marker range check
  skips C4, C8 and CC: those codes belong to DHT, the reserved JPG marker, and DAC, none of which
  is a frame header. It also points at §B.2.2's Figure B.3 for the precision/height/width/
  component-count field order it reads at fixed offsets from the length field. `MmrDecoder`'s
  mode-table comment cited "ISO/IEC 11544 Table 2 / ITU-T T.6 §4"; §4 was a page-header artifact,
  Table 2/T.6 is the Group 4 run-length table rather than the mode table, and ISO/IEC 11544 is
  JBIG1, a different coding standard from T.6 Group 4 MMR. It now cites ITU-T T.6 §2.2.3 and
  Table 1/T.6, the two-dimensional code table that defines the pass, vertical and horizontal
  mode codewords, and drops the JBIG1 reference: the held T.6 text supports the comment on
  its own.
