...
- **Measured behaviour of two candidate differential oracles, Ghostscript and MuPDF (#420).**
  ...
  1.23.10, and 1.23.10 has no `mutool audit`, no `show -r` and few of `clean`'s options, so a
  developer's run would exercise different code from CI's. Four measurements decide how either can
  be invoked at all. Ghostscript sends errors to stderr and warnings to stdout, so a test reading
  only stderr passes every warning-class defect; `-q` suppresses its error report entirely, making
  three damaged fixtures indistinguishable from the valid one; three of five damaged fixtures exit
  0, and a zero-byte file exits 0 in silence, so a run has to prove it did work rather than prove
  it did not complain. MuPDF puts everything on stderr and catches that zero-byte file, while
