...
full-range size. ISO/IEC 24778's data-placement figures (Figures 4 and 5)
are raster diagrams absent from the standard's freely available preview
pages, so the data-layer spiral was derived from, and verified bit-for-bit
against, module matrices generated by zxing-cpp (used purely as an oracle,
never copied); the finder, orientation, reference grid and mode message are
likewise cross-checked against a real decoder, and the whole size range is
exercised end to end by render/rasterize/decode round-trips in the test
suite.
