...
Line grouping is rotation-aware: a rotated page's baseline still joins into one line rather than
opening a new run per glyph. Not implemented yet: `/ToUnicode` and predefined CMaps, Type0 and
Type3 fonts (reported as `FontTypeUnsupported`, code 405, rather than silently producing no text),
`/ActualText`, `/ReversedChars`, and word- and paragraph-aware line grouping. Concretely, the
word-aware gap means no space is synthesised for a positioning gap: `[(Hello) -500 (World)] TJ`
extracts as `HelloWorld`, not `Hello World`, since nothing in the content stream itself is a space
character.
...
| Text extraction (`ExtractText`) | ⚠️ Partial — simple fonts (Type1, MMType1, TrueType) only, positioned per ISO 32000-2 §9.4.4, page content only (not annotation appearances); line grouping is rotation-aware; no space is synthesised for a positioning gap (`[(Hello) -500 (World)] TJ` extracts as `HelloWorld`); `/ToUnicode`, predefined CMaps, Type0/Type3 fonts, `/ActualText`, `/ReversedChars`, and word/paragraph-aware line grouping not yet implemented | ISO 32000-2 §9.4.4 (#98) |
