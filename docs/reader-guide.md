...
`/ReversedChars`, and rotation-, word-, and paragraph-aware line grouping. Concretely, that last
gap means no space is synthesised for a positioning gap: `[(Hello) -500 (World)] TJ` extracts as
`HelloWorld`, not `Hello World`, since nothing in the content stream itself is a space character.
...
| Text extraction (`ExtractText`) | ⚠️ Partial — simple fonts (Type1, MMType1, TrueType) only, positioned per ISO 32000-2 §9.4.4, page content only (not annotation appearances); no space is synthesised for a positioning gap (`[(Hello) -500 (World)] TJ` extracts as `HelloWorld`); `/ToUnicode`, predefined CMaps, Type0/Type3 fonts, `/ActualText`, `/ReversedChars`, and rotation/word/paragraph-aware line grouping not yet implemented | ISO 32000-2 §9.4.4 (#98) |
