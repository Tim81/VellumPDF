...
| --- | --- | --- | --- |
| Ghostscript | 10.07.1 (2026-05-19) | 10.02.1 | Artifex ships no prebuilt Linux binary, so this is a source build from the pinned tag either way; building also keeps it off the apt snapshot dependency in #410 |
| MuPDF | 1.28.0 (2026-06-26) | 1.23.10 | five minor versions behind: `mutool audit`, `grep`, `barcode` and `bake` do not exist in it, nor do `show -r`, `show -L`, or most of `clean`'s options |

MuPDF pins 1.28.0 rather than the newer 1.28.3 because Artifex publishes a Windows zip only for
`x.y.0` releases. One version number then covers Linux and Windows, which is what keeps a
developer's run comparable to CI's.
...
| zero bytes | **0** | **nothing at all**, in both modes |
...
Exit status is not the signal. Three of the five defective fixtures exit 0. Ghostscript's stated
policy is to render broken files and print a warning so that it stays useful as a sanity check, so
the text is the finding and the exit code is not. `-dPDFSTOPONERROR` makes the status mean something,
and every observed failure under it exited **1**, not the 255 the source's error mapping suggests
...
A clean run has to prove it did work. The zero-byte file exits 0 with no output whatsoever, so
"Ghostscript did not complain" is not evidence that Ghostscript read anything. Use `-sDEVICE=bbox`,
which emits `%%BoundingBox` and `%%HiResBoundingBox` on stderr for every page it processed: that
line is the proof-of-work signal, and its absence is what distinguishes an empty file from a clean
one. Prefer it over `nullpage`, which works but appears nowhere in the documentation.
...
gate stays green, so each engine ships with a canary: a known-bad fixture asserted to fail.
...
What it can report is that a file parses, and which of its 83 error and 96 warning classes the file
...
`format error: no objects found`, `warning: PDF stream Length incorrect`.
...
| --- | --- | --- |
| zero bytes | exit 0, silent — **misses it** | exit 1, `no objects found` — **catches it** |
