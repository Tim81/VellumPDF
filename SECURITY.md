...
Dictionary lookup was a linear scan through v2.0, so a `/Encrypt` dictionary carrying very many keys
...
once a dictionary passes 16 entries, so lookup and construction stay linear past that point: building
one directly with a million keys takes about 0.5 s, measured here (#208). Bounding input size remains
the caller's responsibility more generally. The reader bounds indirect-reference nesting and AcroForm
field-tree depth, rejects object-stream cycles, and range-checks every offset taken from a
cross-reference table before using it.
